...
#define CMD_FUNC(n) [n]={(unsigned char *) NAME(n), n}

static cmd_map_t cmd_map[] = {
                              CMD_FUNC(expert_mode), get_gpio),
...
int
i2c_write(unsigned char * params,
          unsigned char * reply,
          int conn_idx)
{
  int reply_len = 0;
...
int
i2c_reg_write(unsigned char * params,
                  unsigned char * reply,
                  int conn_idx)
{
  int reply_len = 0;
...
int
i2c_reg_read(unsigned char * params,
             unsigned char * reply,
             int conn_idx)
{
  int reply_len = 0;
                ...
                int conn_idx)
{
  ...
  if (str_eq(param, (unsigned char *) "m") == 1) {
  ...
  if (str_eq(param, (unsigned char *) "s") == 1) {
